...
public class DepartmentWithHighestAverageSalary {
    public static Map<String,Double> departmentWithHighestAvgSalary(List<Employee1> employees) {
        return employees.stream()
                .collect(Collectors.groupingBy(Employee1::getDepartment,
                        Collectors.averagingDouble(Employee1::getSalary)))
                .entrySet().stream().max(Comparator.comparing(Map.Entry::getValue)).map(entry -> Map.of(entry.getKey(), entry.getValue())).orElse(Map.of());
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(departmentWithHighestAvgSalary(employeeList));
    }
}
